...
t0 = get_ms_of_day(); 	
	...
	printf("%c\t%d\t%d\t%d\t%g\t%g\t%g\t%g\t%g\t Node:N:Core;TotalTime;initcape;initdata;cal;final\n", 's',THREADS,cape_get_node_num(),N, tm4-tm0,tm1-tm0,tm2-tm1,tm3-tm2,tm4-tm3);
	printf("%c\t%d\t%d\t%d\t%ld\t%ld\t%ld\t%ld\t%ld\t Node:N:Core;TotalTime;initcape;initdata;cal;final\n", 'm',THREADS,cape_get_node_num(),N, t4-t0, t1-t0, t2-t1, t3-t2, t4-t3);
}
